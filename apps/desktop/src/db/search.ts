...
async function runFtsSearch(
  ...
  for (const row of rawRows) {
    ...
    let idx = 3;
    if (typeof opts.dateFrom === "number") { snippetWhere += ` AND COALESCE(m.created_at, 0) >= $${idx}`; snippetParams.push(opts.dateFrom); idx += 1; }
    if (typeof opts.dateTo === "number") { snippetWhere += ` AND COALESCE(m.created_at, 0) <= $${idx}`; snippetParams.push(opts.dateTo); }
    ...
    rows.push({ conversation_id: row.conversation_id, title: row.title, source: row.source, snippet: snippets[0] ?? "", snippets, created_at: row.created_at, last_occurrence: row.last_occurrence, occurrence_count: row.occurrence_count, message_match_count: row.message_match_count, rank: row.rank, first_match_message_id: row.first_match_message_id });
  }
  return { rows, totalMatches: countRows[0]?.total ?? 0, totalOccurrences: totalOccRows[0]?.total ?? 0 };
}
  ...
  if (mode === "semantic" || mode === "hybrid") {
    return withDbLock(async () => {
      ...
      for (const s of scored) {
        ...
        if (s.message_created_at > entry.last_occurrence) entry.last_occurrence = s.message_created_at;
      ...
      if (mode === "hybrid") {
        ...
        for (const r of semanticRows as any[]) merged.set(r.conversation_id, { ...r, ftsRank: undefined });
        for (const fr of ftsResult.rows as any[]) {
          ...
          if (existing) {
            ...
            existing.message_match_count = Math.max(existing.message_match_count, fr.message_match_count);
            // hybrid rank blend
        ...
        (semanticRows as any[]).sort((a: any, b: any) => a.rank - b.rank || b.last_occurrence - a.last_occurrence);
      } else {
        (semanticRows as any[]).sort((a: any, b: any) => a.rank - b.rank || b.last_occurrence - a.last_occurrence);
      }
