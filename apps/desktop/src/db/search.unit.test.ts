...
vi.mock("../lib/vector", async (orig) => {
  ...
  return {
    ...
    embed: vi.fn((t: string) => {
      ...
      if (t.toLowerCase().includes("vacation") || t.toLowerCase().includes("holiday"))
        return [1, 0, 0, 0];
  ...
  it("semantic mode filters by similarity threshold and source/date", async () => {
    ...
    mockSelect.mockResolvedValueOnce([
      ...
      },
      {
        message_id: "m3",
        conversation_id: "c1",
        content: "holiday getaway",
        message_created_at: 150,
        title: "Trip",
        source: "claude",
        conv_created_at: 90,
      },
